...
class Rule(object):
    ...
    def __init__(self, pattern, **param):
        ...
        self.pattern = pattern[:-1] if pattern.endswith('/'
                ) else pattern
        ...
        for i in range(len(self.matches)):
            # It is valid for routes to have . characters in them, so we need to escape it in order
            # to prevent it form screwing up the RegEx.
            self.regex = self.regex.replace('.','\\.').replace(':' + self.matches[i],
                    '([^/.]+)')
    ...
    def url_for(self, controller, **param):
        ...
        ep = '&'.join(['%s=%s' % (k, v) for (k, v) in param.items() if k
                       not in self.param])
...
class Router:
    ...
    class __impl:
        ...
        def __init__(self):
            self.__routing_root = {'controller': 'home',
                                   'action': 'index'}
            self.__routing_table = []

            # used to store default pattern (but match last)

            self.__routing_table_fallback = [Rule('/:controller/:action'
                    ), Rule('/:controller')]
        ...
        def root(self, **map):
            ...
            self.__routing_root['controller'] = map.get('controller',
        ...
        def resolve(self, url):
            ...
            if ret is None:  # fallback
                ret = self.__resolve_by_table(url,
